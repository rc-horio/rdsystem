...
type MultiBlockLayoutModel = {
  blocks: BlockLayoutResolved[];
  totalWidthM: number;
  totalHeightM: number;
};

function buildMultiBlockLayoutModel(area: Area): MultiBlockLayoutModel | null {
  ...
  const blockById = new Map(
    blocks.map((b) => [b.id, b] as const)
  );

  const blockIndexById = new Map(
    blocks.map((b, i) => [b.id, i] as const)
  );

  const resolved: BlockLayoutResolved[] = [];

  // 一度、各行の高さと横幅を計算しつつ「下からの積み上げ」で全体高さを出す。
  // rows[0] が一番下、その上に rows[1] ... という想定。
  ...
  layout.rows.forEach((row, rowIndex) => {
    ...
    row.block_ids.forEach((bid, bi) => {
      ...
      // 同一行内では「下端」を揃える（基準点は左下）。
      // 今の rowTop は「行コンテナの上端」なので、ブロックごとの高さ差分だけ y を補正する。
  ...
  if (!Number.isFinite(totalWidthM) || !Number.isFinite(totalHeightM) || totalWidthM <= 0 || totalHeightM <= 0) {
    return null;
  }
